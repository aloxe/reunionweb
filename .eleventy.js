...
module.exports = (eleventyConfig) => {
  ...
  eleventyConfig.addTransform("htmlmin", async function (source, output_path) {
    ...
    if(output_path.endsWith("feed.xml")) {
      const rssResult = await minify_html(source, {
      ...
      console.log(`MINIFY ATOM FEED ${output_path}`, source.length, `→`, rssResult.length, `(${((1 - (rssResult.length / source.length)) * 100).toFixed(2)}% reduction)`);
      return rssResult;
    }
